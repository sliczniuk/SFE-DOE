...
%for ii = 1:num_of_exp
    ...
    T0homog                 = OPT_solver.variable(numel(OP_change))';
                              OPT_solver.subject_to( 30 <= T0homog <= 40 );
    ...
    feedTemp                = repmat(T0homog,OP_change_Time/timeStep,1) + 273;
    feedTemp                = feedTemp(:)';
    ...
    Fisher_cost = -log(myDet(FI));
    FF = Function('FF',{[T0homog, Flow, Parameters_sym_t']},{Fisher_cost});
    ...
    D_opt = Fisher_cost + ControlEffort_P + ControlEffort_T;
    
    %% Defin intial guesses
    %T0 = linspace(30,40,numel(T0homog))+273;
    T0 = ( (40-30).*rand(1,numel(T0homog)) + 30 );
...
for i = 1%:3
    ...
    figure(i+1)
    yyaxis right
    stairs([0 OP_change], [KOUT_MATRIX(II,1:numel(OP_change)) KOUT_MATRIX(II,numel(OP_change))]-273, 'LineWidth', 2)
...
Fisher_nopt            = FF([T0homog-273, Flow, cell2mat(Parameters(which_theta))'])

%% Set operating conditions
T0homog                 = KOUT(1:numel(T0))+273;
...
Fisher_opt             = FF([T0homog-273, Flow, cell2mat(Parameters(which_theta))'])
...
figure()
hold on
stairs([0 OP_change],[KOUT(1:numel(OP_change)) KOUT(numel(OP_change))], 'LineWidth', 2)
stairs([0 OP_change],[T0 T0(end)], 'LineWidth', 2)
hold off
ylabel('Temperture C')
xlabel('Time min')
%legend('Optimized solution','Inital guess', 'Location','Best', 'box', 'off')
