...
This study investigates the extraction of valuable components from biomass using supercritical carbon dioxide as a solvent. The interest is in essential oils extracted from chamomile flowers in cylindrical extractors operated in a semi-batch mode. The extraction process is described by a distributed-parameter model, and set of empirical correlations. This work aims \underline{to validate the empirical correlations by designing a new experiment} with dynamically changing operating conditions. %The process model includes empirical correlations, which describe the relation between the extraction kinetics parameters and the operating conditions. 
\end{tcolorbox}
