function Sdot = modelSFE_Corr_sensitivity(X, p, mask, dt)
    % (t, x, u, parameters)
    ...
    s             =     reshape(X(3*nstages_index+4:end), 3*nstages_index+3, 3);
   ...
   - 1            ./  ( 1 - epsi .* mask ) .* d_cons_CF_dz   + ...
    ...
    Jtheta_P =  Jtheta_p(:,[44:46]+3);                                           % select jacobians related to specific p from which_theta
